...
const EditarIE = ({ show, handleClose, ie, refreshData }) => {
    ...
    const initialValues = {
        NO_ENTIDADE: ie.NO_ENTIDADE || "",
        NO_REGIAO: ie.NO_REGIAO || "",
        NO_UF: ie.NO_UF || "",
        NO_MUNICIPIO: ie.NO_MUNICIPIO || "",
        NO_MESORREGIAO: ie.NO_MESORREGIAO || "",
        NO_MICRORREGIAO: ie.NO_MICRORREGIAO || "",
        QT_MAT_BAS: ie.QT_MAT_BAS || "",
    };

    const handleSubmit = async (values, { setSubmitting }) => {
        try {
            const response = await fetch(`http://localhost:3000/ies/${ie.id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(values),
            });

            if (!response.ok) throw new Error("Erro ao atualizar a IE");
